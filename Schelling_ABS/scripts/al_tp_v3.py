...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr  1 14:18:09 2026
...
for loop in range(total_loops):
    ...
    n_added_kde = 50
    x_val, _ = samp(n_added_kde, return_is_acting=True)
    ...
    for l in range(n_features):
        ...
        b_min = np.min(feature_col)
        b_max = np.max(feature_col)
        
        # On isole l'historique (les anciens points x_doe) pour mesurer la distance
        historique_shap = feature_col[n_added_kde:]
        
        # 2. Entraîner le KDE NORMALEMENT sur les vrais points
        kde = gaussian_kde(feature_col, bw_method=0.1)
        kdelr = gaussian_kde(feature_col, bw_method=0.2)

        # 3. Évaluer la densité réelle
        d_real = kde(feature_col)

        # =======================================================
        # 4. VOTRE FORCE DE RÉPULSION BASÉE SUR LA DISTANCE
        # =======================================================
        # Calcul de la distance absolue entre chaque point et le point historique le plus proche
        dist_matrix = np.abs(feature_col[:, np.newaxis] - feature_col)
        dist_matrix[dist_matrix==0]=1
        min_distances = np.min(dist_matrix, axis=1)
        
        # Votre formule : 2 * exp(-distance)
        # J'ai ajouté un multiplicateur '10.0' car les valeurs SHAP sont souvent toutes petites (ex: 0.05). 
        # Sans cela, l'exponentielle ne baisserait pas assez vite !
        
        # 5. Additionner le tout
        d_left_ghosts = kdelr(2 * b_min - feature_col)
        d_right_ghosts = kdelr(2 * b_max - feature_col)
        densities = d_real + d_left_ghosts + d_right_ghosts  
        # --- NORMALISATION OBLIGATOIRE POUR LE KRIGEAGE ---
        # On calcule le 90ème percentile pour éviter d'être écrasé par un pic aberrant
        p_max = np.percentile(densities, 90)
        
        # On divise tout par p_max et on coupe ce qui dépasse à 1.0
        if p_max > 0:
            densities = np.clip(densities, 0, p_max) / p_max

        # On calcule la largeur du domaine de cette variable
        largeur_domaine = b_max - b_min
        
        # On veut que la pénalité tombe à zéro à environ 5% de la largeur du domaine
        # (Pour que la répulsion soit vraiment locale)
        facteur_chute = 3.0 / (0.05 * largeur_domaine) if largeur_domaine > 0 else 1.0
        
        repulsion_distance =  0.2*np.exp(-facteur_chute * min_distances)
        densities =  densities+ repulsion_distance

        density_matrix[l, :] = densities
        
# =============================================================================
#         # ==========================================
#         # --- PETIT PLOT DE DÉBOGAGE (KDE Soft Folding) ---
#         # ==========================================
#         # Afficher uniquement pour la 1ère variable à la 1ère boucle
#         import matplotlib.pyplot as plt
#         
#         # Pour comparer, on calcule rapidement le KDE classique (qui fuit)
#         kde_naif = gaussian_kde(feature_col[:n_added_kde]   ,bw_method=0.1)
#         
#         plt.figure(figsize=(8, 5))
#         marge = (b_max - b_min) * 0.2
#         x_eval = np.linspace(b_min - marge, b_max + marge, 300)
#         
#         # 1. Tracer le KDE naïf (Gris, pointillés)
#         # plt.scatter(feature_col, kde_naif(feature_col), color='gray', linestyle='--', lw=2, label='KDE Naïf (Fuite de masse)')
#         
#         # 2. Tracer le KDE Soft Folding (Rouge)
#         plt.scatter(feature_col, densities, color='red', lw=2.5, label=f'KDE Soft Folding ')
#         
#         # 3. Afficher les vrais points SHAP
#         plt.scatter(feature_col, np.zeros_like(feature_col), color='blue', alpha=0.6, s=40, label='Vrais points SHAP', zorder=5)
#         plt.scatter(feature_col[-1], np.zeros(1), color='red', alpha=0.6, s=40, label='Vrais points SHAP', zorder=5)
# 
#         # 4. Afficher les murs (frontières)
#         plt.axvline(b_min, color='black', linestyle=':', lw=2, label='Limites empiriques')
#         plt.axvline(b_max, color='black', linestyle=':', lw=2)
#         
#         plt.title(f"Débogage Soft Folding - Variable", fontsize=14, fontweight='bold')
#         plt.xlabel("Valeur SHAP", fontsize=12)
#         plt.ylabel("Densité brute", fontsize=12)
#         plt.legend()
...
# =============================================================================
    ...
    epsilon = 0.05
    y_min_observed = np.min(density_mean_matrix)+epsilon
            
    density_mean_matrix1 = density_mean_matrix[:n_added_kde]
    density_mean_matrix2 = density_mean_matrix[n_added_kde:]
    
    # The Multi-Fidelity surrogate predicting the SHAP density
    surro = MFK(theta0=[1e-2], print_global=False, eval_noise=True, nugget=1e-7, design_space=design_space_reduced)
    ...
    max_sigma = np.max(sigma_samples) if np.max(sigma_samples) > 1e-12 else 1.0
        ...
        # --- DÉFINITION DE LA BULLE D'INTÉRÊT p(x) ---
    bnds = design_space_reduced.get_num_bounds()
    lower_b = np.array([b[0] for b in bnds])
    upper_b = np.array([b[1] for b in bnds])
    
    # Le centre parfait de l'hypercube 5D
    mu_px = (lower_b + upper_b) / 2.0
    
    # L'étalement de la cloche. 
    # En divisant par 4.0, la cloche s'écrase sur les bords (p(x) y vaudra environ 0.13)
    # Si vous voulez fuir les bords encore plus violemment, divisez par 6.0 !
    sigma_px = (upper_b - lower_b) / 4.0
    bnds = design_space_reduced.get_num_bounds()
    mu_px = np.array([(b[0] + b[1]) / 2.0 for b in bnds])
    sigma_px = np.array([(b[1] - b[0]) / 4.0 for b in bnds])
    
    # Voici nos points d'intégration u_i (qui suivent p(u))
    np.random.seed(42)
    U_integr = np.random.normal(loc=mu_px, scale=sigma_px, size=(100, 5))
    U_integr = np.clip(U_integr, [b[0] for b in bnds], [b[1] for b in bnds])
    ...
    def obj_fun(x):
        ...
        # C. LE COUP DE GÉNIE : Pondération locale par p(x)
        # Calcule la valeur de la cloche gaussienne au point x_target
        p_x = np.exp(-0.5 * np.sum(((x_2d[0] - mu_px) / sigma_px)**2))
        
        # On annule l'attrait de sigma si on est près des murs
        weighted_sigma = sigma_norm * p_x
        
        # C. Combined Hybrid Score
        hybrid_score = (alpha * ei_norm) + ((1 - alpha) * weighted_sigma)
...
save_database_to_csv(DATABASEx, DATABASEy, feature_names, filename=f"../data/processed/v3/al_results_loop_{loop}.csv")
# -----------------------
...
x_doe_100, _ = samp_100(40, return_is_acting=True)
...
print("-" * 95)
print(f"{'Pure 100-point LHS DoE':<26}| {metrics_doe_input['Normalized_Entropy']:<14.4f} | {metrics_doe_input['CV_NND']:<14.4f}| {metrics_doe_shap['Normalized_Entropy']:<14.4f} | {metrics_doe_shap['CV_NND']:<14.4f}")
...
# ==========================================
# 6. VISUALIZATION: 2D CROSS-SECTION OF THE 5D DESIGN SPACE FRONTIER
# ==========================================
print("\nGenerating 2D Input Space Comparison Map with MLP Frontier...")
from matplotlib.lines import Line2D

# --- Feature indices based on the 5D DesignSpace ---
# [0:"Number of Types", 1:"Density", 2:"Intolerance threshold", 3:"Map size", 4:"Perception distance"]
idx_density = 1
idx_intolerance = 2
...
# --- CALCULATE THE 5D MLP DECISION FRONTIER ---
...
# Use 200x200 resolution to prevent RAM explosion when scaling 5D data
dd, ii = np.meshgrid(np.linspace(d_min-0.1, d_max+0.1, 200), np.linspace(i_min-0.1, i_max+0.1, 200))

# To query the 5D MLP, we MUST feed it 5 features. 
# We vary the 2 we want to plot, and freeze the other 3 at their empirical mean.
mean_features = np.mean(DATABASEx, axis=0)

# Initialize the array with exactly 5 columns
...
for j in range(5): 
    if j == idx_density:
        ...
        mesh_points[:, j] = mean_features[j] # Freeze the other 3 variables

# Scale and predict the grid, grabbing ONLY column 1 (Probability of Convergence = True)
...
# Formatting
fig_map.suptitle("Input Space Exploration vs. MLP Decision Boundary\n(2D slice of 5D space, other parameters frozen at mean)", fontsize=16, fontweight='bold')
plt.tight_layout()

# Save and Show
output_map_file = "../figures/analysis/input_space_comparison_5D_frontier.png"
