...
def plot_target_distribution():
    print("Generating Target (Segregation) Distribution...")
    fig, ax = plt.subplots(figsize=(10, 6))
    
    df_lhs, df_sur, df_sur_shap, df_v5 = get_data_for_seed(42)
    
    # We only want the ADDED points for AL methods
    # Segregation is column 5 (0-indexed)
    prices_lhs = df_lhs.iloc[:, 5].values
    prices_sur = df_sur.iloc[:, 5].values[30:]
    prices_v5 = df_v5.iloc[:, 5].values[30:]
    prices_shap = df_sur_shap.iloc[:, 5].values[30:]
    ...
    ax.set_title("Distribution of Explored Targets (Segregation)", fontsize=16, fontweight='bold')
    ax.set_xlabel("Segregation (Target)", fontsize=14)
...
def plot_dual_space():
    ...
    out_img = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "figures", "analysis", "dual_space_efficiency_point.png"))
    plt.savefig(out_img, dpi=300)
    plt.close()
