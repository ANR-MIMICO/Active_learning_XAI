...
save_database_to_csv(DATABASEx, DATABASEy, feature_names, filename=f"../data/processed/v1/al_results_loop_{loop}.csv")
# -----------------------
...
output_file = "doe_evolution_with_density.gif"
...
output_shap_file = "shap_beeswarm_evolution.gif"
...
def compute_shap_uniformity(shap_values, n_bins=15):
    """
    Computes uniformity metrics for points in the SHAP explanation space.
    """
    n_samples, n_features = shap_values.shape
    ...
    entropies = []
    for i in range(n_features):
        hist, _ = np.histogram(shap_values[:, i], bins=n_bins, density=False)
    ...
    nn.fit(shap_values)
    distances, _ = nn.kneighbors(shap_values)
    nnd = distances[:, 1] 
...
print("\n--- Computing Final SHAP Uniformity Comparison ---")
...
metrics_doe = compute_shap_uniformity(shap_doe_100)
DATABASEx = DATABASEx[:100]
# 2. Evaluate Final Active Learning Dataset
print(f"2/2 Evaluating Active Learning dataset (N={len(DATABASEx)})...")
...
metrics_al = compute_shap_uniformity(shap_al_final)

# 3. Print Results
print("\n" + "="*65)
print(f"{'Dataset':<25} | {'Normalized Entropy (↑)':<22} | {'CV NND (↓)':<15}")
print("-" * 65)
print(f"{'Pure 100-point DoE':<25} | {metrics_doe['Normalized_Entropy']:<22.4f} | {metrics_doe['CV_NND']:<15.4f}")
print(f"{f'Active Learning (N={len(DATABASEx)})':<25} | {metrics_al['Normalized_Entropy']:<22.4f} | {metrics_al['CV_NND']:<15.4f}")
print("="*65)
print("↑ Higher Entropy = More uniform distribution across SHAP bins.")
print("↓ Lower CV NND   = Points are spread out more evenly without dense clusters.")