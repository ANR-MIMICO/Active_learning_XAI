...
def plot_boundary():
    ...
    colors = {"LHS": "black", "SUR": "blue", "V4": "red", "SUR_SHAP": "green"}
    labels = {"LHS": "LHS", "SUR": "Space-US", "V4": "Hybrid-US", "SUR_SHAP": "SHAP-US"}
    ...
    for m in methods:
        ...
        # Wait, in paper_benchmark, I only renamed the CSV and left the folders `tmp_v4_seed` intact?
        csv_files = glob.glob(os.path.join(results_dir, f"tmp_{m.lower()}_*", "al_database.csv"))
        if len(csv_files) == 0:
