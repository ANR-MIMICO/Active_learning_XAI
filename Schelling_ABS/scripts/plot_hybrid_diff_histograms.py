...
def plot_hybrid_diff():
    ...
    for seed in seeds:
        ...
        path_v6 = os.path.join(results_dir, f"tmp_v6_dyn_{seed}", "al_database_loop_49.csv")
        if not os.path.exists(path_v6): path_v6 = os.path.join(results_dir, f"tmp_v6_dyn_{seed}", "al_database.csv")
        
        if os.path.exists(path_lhs) and os.path.exists(path_v6):
            df_lhs_list.append(pd.read_csv(path_lhs))
            df_v6_list.append(pd.read_csv(path_v6))
    ...
    for i, var_name in enumerate(var_names):
        ...
        X_v6 = df_v6.iloc[:, i].values
        
        hist_lhs, bin_edges = np.histogram(X_lhs, bins=bins_to_use, density=False)
        hist_v6, _ = np.histogram(X_v6, bins=bins_to_use, density=False)
        
        hist_lhs = hist_lhs / len(X_lhs)
        hist_v6 = hist_v6 / len(X_v6)
