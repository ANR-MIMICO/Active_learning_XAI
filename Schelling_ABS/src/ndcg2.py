...
file_path = r"C:\Users\psaves\Desktop\Moncef\Classification\iris.tsv"
iris_df = pd.read_csv(file_path, sep="\t")
iris_np = iris_df.to_numpy()

print(iris_df.head())
print("Numpy shape:", iris_np.shape)
...
file_list = [f for f in glob.glob("../data/processed/*.npy") if "_"+str(inputval) in f]
...
# 
# # Get a list of all .npy files in the directory
# file_list = [f for f in glob.glob("../data/processed/*.npy") if "_0" in f]
# # Load each .npy file into a list
# arrays = [np.load(file) for file in file_list]
# 
# from itertools import combinations
