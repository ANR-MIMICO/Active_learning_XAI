...
for loop in range(total_loops):
    ...
    for l in range(n_features):
        ...
        # Vous pouvez utiliser un bw_method fixe (ex: 0.15) pour garder de belles collines
        kde = gaussian_kde(feature_col[n_added_kde:], bw_method=0.1)
        
        # 3. Évaluer la densité réelle
        d_real = kde(feature_col)
        
        # 4. Évaluer la densité des "queues" qui ont fui de l'autre côté du miroir
        d_left_ghosts = kde(2 * b_min - feature_col)
        d_right_ghosts = kde(2 * b_max - feature_col)
        
        # 5. Additionner le tout (C'est ici que le "Soft Folding" opère mathématiquement !)
        densities = d_real + (alpha_fold * d_left_ghosts) + (alpha_fold * d_right_ghosts)
...
feature_names = ["Density", "Intolerance threshold"]
...
dd, ii = np.meshgrid(np.linspace(d_min-0.1, d_max+0.1, 1000), np.linspace(i_min-0.1, i_max+0.1, 1000))

# To query the 5D MLP, we MUST feed it 5 features. 
# We vary the 2 we want to plot, and freeze the other 3 at their mean.
mean_features = np.mean(DATABASEx, axis=0)
mesh_points = np.zeros((dd.size, 2)) # <--- MUST BE 5 to match the trained MLP

for j in range(2): # <--- MUST loop through all 5
    if j == idx_density:
...
# Save and Show
output_map_file = "input_space_comparison_2D_frontier.png"
