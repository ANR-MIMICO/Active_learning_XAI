...
if os.path.exists(path):
    ...
    content = content.replace('methods = ["LHS", "SUR", "SUR_SHAP", "V5"]', 'methods = ["LHS", "SUR_SHAP", "V6_SUR", "V6_DYN"]')
    content = content.replace('methods = ["LHS", "SUR", "SUR_SHAP", "V5", "V6_SUR", "V6_DYN"]', 'methods = ["LHS", "SUR_SHAP", "V6_SUR", "V6_DYN"]')
    content = content.replace('colors = {"LHS": "black", "SUR": "blue", "SUR_SHAP": "green", "V5": "red"}', 'colors = {"LHS": "black", "SUR_SHAP": "green", "V6_SUR": "blue", "V6_DYN": "red"}')
    ...
    content = content.replace('"V5": "Dynamic-US"', '"V6_SUR": "IMSE-US",\n        "V6_DYN": "Dynamic-US"')
    # Prune old V5/SUR if they were explicitly there
    import re
    content = re.sub(r'"SUR": "Space-US",\s*', '', content)
...
if os.path.exists(path):
    ...
    content = content.replace('"V5": "Dynamic-US"', '"V6_SUR": "IMSE-US",\n        "V6_DYN": "Dynamic-US"')
    content = re.sub(r'"SUR": "Space-US",\s*', '', content)
    content = content.replace('cmap_dict = {"LHS": "Greys", "SUR": "Blues", "SUR_SHAP": "Greens", "V5": "Reds"}', 'cmap_dict = {"LHS": "Greys", "SUR_SHAP": "Greens", "V6_SUR": "Blues", "V6_DYN": "Reds"}')
...
if os.path.exists(path):
    with open(path, "r", encoding="utf-8") as f: content = f.read()
    content = content.replace('methods = ["LHS", "SUR", "V5", "SUR_SHAP"]', 'methods = ["LHS", "SUR_SHAP", "V6_SUR", "V6_DYN"]')
    content = content.replace('methods = ["LHS", "SUR", "V5", "SUR_SHAP", "V6_SUR", "V6_DYN"]', 'methods = ["LHS", "SUR_SHAP", "V6_SUR", "V6_DYN"]')
    content = content.replace('colors = {"LHS": "black", "SUR": "blue", "V5": "red", "SUR_SHAP": "green"}', 'colors = {"LHS": "black", "SUR_SHAP": "green", "V6_SUR": "blue", "V6_DYN": "red"}')
    ...
    content = content.replace('labels = {"LHS": "LHS", "SUR": "Space-US", "V5": "Dynamic-US", "SUR_SHAP": "SHAP-US"}', 'labels = {"LHS": "LHS", "SUR_SHAP": "SHAP-CS", "V6_SUR": "IMSE-US", "V6_DYN": "Dynamic-US"}')
...
if os.path.exists(path):
    ...
    content = content.replace('''    methods = [("lhs", "LHS"), 
               ("sur", "Space-US"), 
               ("sur_shap", "SHAP-US"), 
               ("v5", "Hybrid-US (V5)")]''', '''    methods = [("lhs", "LHS"), 
               ("sur_shap", "SHAP-CS"), 
               ("v6_sur", "IMSE-US"),
               ("v6_dyn", "Dynamic-US")]''')
    with open(path, "w", encoding="utf-8") as f: f.write(content)
...
if os.path.exists(path):
    ...
    content = content.replace('methods = ["lhs", "sur", "sur_shap", "v5"]', 'methods = ["lhs", "sur_shap", "v6_sur", "v6_dyn"]')
    content = content.replace('methods = ["lhs", "sur", "sur_shap", "v5", "v6_sur", "v6_dyn"]', 'methods = ["lhs", "sur_shap", "v6_sur", "v6_dyn"]')
    content = content.replace('titles = {"lhs": "LHS", "sur": "Space-US", "sur_shap": "SHAP-US", "v5": "Hybrid-US (V5)"}', 'titles = {"lhs": "LHS", "sur_shap": "SHAP-CS", "v6_sur": "IMSE-US", "v6_dyn": "Dynamic-US"}')
    content = content.replace('colors = {"lhs": "black", "sur": "blue", "sur_shap": "green", "v5": "red"}', 'colors = {"lhs": "black", "sur_shap": "green", "v6_sur": "blue", "v6_dyn": "red"}')
    with open(path, "w", encoding="utf-8") as f: f.write(content)
...
if os.path.exists(path):
    with open(path, "r", encoding="utf-8") as f: content = f.read()
    content = content.replace("for method in ['lhs', 'sur', 'sur_shap', 'v5']:", "for method in ['lhs', 'sur_shap', 'v6_sur', 'v6_dyn']:")
    ...
    content = content.replace('''        if method == 'lhs': m_title = "LHS"
        ...
        elif method == 'v5': m_title = "Hybrid-US (V5)"
        color = 'magenta' if method == 'lhs' else 'cyan' if method == 'sur' else 'lime' if method == 'sur_shap' else 'red'
